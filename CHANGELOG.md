...
- **Final scientific finalization (2026-08-13):** [docs/PHI_FINAL_SCIENTIFIC_STATUS.md](docs/PHI_FINAL_SCIENTIFIC_STATUS.md) (23-section authoritative arbitration record), [docs/PHI_FINAL_RESEARCH_REPORT.md](docs/PHI_FINAL_RESEARCH_REPORT.md) (publication-grade research narrative), and [docs/RELEASE_STATUS.md](docs/RELEASE_STATUS.md). **Verdict: B — NOT CLEARED — REPAIR REQUIRED.** New finding (`scripts/phase4_trend_plus_noise_reverification.py`, `results/phase4_repair_validation/trend_plus_noise_reverification.json`): the repair's per-DGP FPR gate was reported passing for `trend_plus_noise` at exactly its 0.07 threshold from a single seed (`n=100`); an 8-run, 1,200-series multi-seed replication using the existing unmodified harness shows the true FPR is **≈0.108 (95% CI [0.091, 0.126])**, decisively above threshold — the gate does not robustly pass. Combined with the granularity-gate failure above, the repair's five-part acceptance gate has two confirmed failures, not one — status corrected from "validation pending" to **validation failed**. Also newly documented: the mandated secondary/tertiary (GARCH, IAAFT) surrogate audits are implemented but unused in the validation battery (calibration evidence rests on the primary block-permutation surrogate only), and the φ-vs-5/8 rational-specificity test is implemented but exercised only once in a unit test, never against the null-DGP suite. No methodology, threshold, DGP, or surrogate was changed. Engineering re-verified: 347 tests passed, 98% branch coverage, Ruff and mypy clean.
