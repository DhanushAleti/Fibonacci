...
```
SCIENCE STATUS:       B — NOT CLEARED — REPAIR REQUIRED
CONFIRMATORY STATUS:  fail-closed; NOT authorized to run
REAL DATA:            NOT RUN
```

See [docs/PHI_FINAL_SCIENTIFIC_STATUS.md](docs/PHI_FINAL_SCIENTIFIC_STATUS.md) for the authoritative
scientific verdict and [docs/PHI_FINAL_RESEARCH_REPORT.md](docs/PHI_FINAL_RESEARCH_REPORT.md) for the
full research narrative — the original confirmatory estimand failed adversarial synthetic validation
(false-positive rate 1.0 across 13 null processes; mathematically explained by Jensen's inequality); an
arbitrated repair substantially improves calibration but does not currently pass its own five-part
acceptance gate (granularity and per-DGP false-positive checks both fail or do not survive replication).
No claim about φ in real markets is made.
